...
{
  union block *header = arch_buffer_next ();
...
{
  union block *header = arch_buffer_next ();
fprintf(stderr, "DEBUG: write_short_name: header #%x\n", header);
  memset (header->buffer, 0, sizeof (union block));
...
create_archive (void)
{
  ...
  {
    const char *name;
    fprintf(stderr, "DEBUG: create_archive .. norm dump_file\n");
    while ((name = name_next (1)) != NULL) {
      fprintf(stderr, "DEBUG: create_archive: %s\n", name); 
...
fprintf(stderr, "DEBUG: dump_file0: %s\n", name);
