...
#ifndef __MEM__SHMEM_CRYPT_RSA_H__
#define __MEM__SHMEM_CRYPT_RSA_H__
...
#define RSA_ERR_RSA_KEY_GEN_FAILED                    -0x4180  /**< Something failed during generation of a key. */
#define RSA_ERR_RSA_KEY_CHECK_FAILED                  -0x4200  /**< Key failed to pass the library's validity check. */
#define RSA_ERR_RSA_PUBLIC_FAILED                     -0x4280  /**< The public key operation failed. */
#define RSA_ERR_RSA_PRIVATE_FAILED                    -0x4300  /**< The private key operation failed. */
#define RSA_ERR_RSA_VERIFY_FAILED                     -0x4380  /**< The PKCS#1 verification failed. */
#define RSA_ERR_RSA_OUTPUT_TOO_LARGE                  -0x4400  /**< The output buffer for decryption is not large enough. */
#define RSA_ERR_RSA_RNG_FAILED                        -0x4480  /**< The random generator failed to generate non-zeros. */
 ...
 *                 shrsa_shrsaes_xxx or shrsa_shrsassa_xxx functions.
 *
 * \note           The chosen hash is always used for OEAP encryption.
 *                 For PSS signatures, it's always used for making signatures,
 *                 but can be overriden (and always is, if set to
 *                 RSA_MD_NONE) for verifying them.
 */
 ...
 *                 the RSA context.
 *
 * \return         0 if successful, or an RSA_ERR_RSA_XXX error code
 */
 ...
 *                 of ctx->N (eg. 128 bytes if RSA-1024 is used).
 */
int shrsa_shrsaes_pkcs1_v15_encrypt( shrsa_context *ctx,
                                 int (*f_rng)(void *, unsigned char *, size_t),
                                 void *p_rng,
                                 int mode, size_t ilen,
                                 const unsigned char *input,
 ...
 *                 of ctx->N (eg. 128 bytes if RSA-1024 is used).
 */
int shrsa_shrsaes_oaep_encrypt( shrsa_context *ctx,
                            int (*f_rng)(void *, unsigned char *, size_t),
                            void *p_rng,
                            int mode,
 ...
 *                 an error is thrown.
 */
int shrsa_shrsaes_pkcs1_v15_decrypt( shrsa_context *ctx,
                                 int (*f_rng)(void *, unsigned char *, size_t),
                                 void *p_rng,
                                 int mode, size_t *olen,
                                 const unsigned char *input,
 ...
 *                 an error is thrown.
 */
int shrsa_shrsaes_oaep_decrypt( shrsa_context *ctx,
                            int (*f_rng)(void *, unsigned char *, size_t),
                            void *p_rng,
                            int mode,
                            const unsigned char *label, size_t label_len,
 ...
 * \param p_rng    RNG parameter
 * \param mode     RSA_PUBLIC or RSA_PRIVATE
 * \param md_alg   a RSA_MD_* (use RSA_MD_NONE for signing raw data)
 * \param hashlen  message digest length (for RSA_MD_NONE only)
 * \param hash     buffer holding the message digest
 * \param sig      buffer that will hold the ciphertext
 *
 * \return         0 if the signing operation was successful,
 *                 or an RSA_ERR_RSA_XXX error code
 *
 * \note           The "sig" buffer must be as large as the size
 *                 of ctx->N (eg. 128 bytes if RSA-1024 is used).
 *
 * \note           In case of PKCS#1 v2.1 encoding, see comments on
 * \note           \c shrsa_shrsassa_pss_sign() for details on md_alg and hash_id.
 */
 ...
 *                 of ctx->N (eg. 128 bytes if RSA-1024 is used).
 */
int shrsa_shrsassa_pkcs1_v15_sign( shrsa_context *ctx,
                               int (*f_rng)(void *, unsigned char *, size_t),
                               void *p_rng,
                               int mode,
                               md_type_t md_alg,
 ...
 */
int shrsa_shrsassa_pss_sign( shrsa_context *ctx,
                         int (*f_rng)(void *, unsigned char *, size_t),
                         void *p_rng,
                         int mode,
                         md_type_t md_alg,
 ...
 *
 * \note           In case of PKCS#1 v2.1 encoding, see comments on
 *                 \c shrsa_shrsassa_pss_verify() about md_alg and hash_id.
 */
 ...
 *                 of ctx->N (eg. 128 bytes if RSA-1024 is used).
 */
int shrsa_shrsassa_pkcs1_v15_verify( shrsa_context *ctx,
                                 int (*f_rng)(void *, unsigned char *, size_t),
                                 void *p_rng,
                                 int mode,
                                 md_type_t md_alg,
 ...
 */
int shrsa_shrsassa_pss_verify( shrsa_context *ctx,
                           int (*f_rng)(void *, unsigned char *, size_t),
                           void *p_rng,
                           int mode,
                           md_type_t md_alg,
 ...
 */
int shrsa_shrsassa_pss_verify_ext( shrsa_context *ctx,
                               int (*f_rng)(void *, unsigned char *, size_t),
                               void *p_rng,
                               int mode,
                               md_type_t md_alg,
 ...
 *
 * \return         O on success,
 *                 RSA_ERR_MPI_MALLOC_FAILED on memory allocation failure
 */
 ...
 */
int shrsa_self_test( int verbose );


#endif /* __MEM__SHMEM_CRYPT_RSA_H__ */
