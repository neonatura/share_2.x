...
typedef struct _x509_crt
{
    ...
    x509_sequence subject_issuer_names;    /**< Optional list of Subject Alternative Names (Only dNSName supported). */
 ...
 */
int x509write_crt_set_serial( x509write_cert *ctx, const shmpi *serial );
...
int x509_get_uid( unsigned char **p, const unsigned char *end, x509_buf *uid, int n );
int x509_get_version( unsigned char **p, const unsigned char *end, int *ver );
int x509_get_dates( unsigned char **p, const unsigned char *end, x509_time *from, x509_time *to );
int x509_get_crt_ext( unsigned char **p, const unsigned char *end, x509_crt *crt );

int x509_info_subject_alt_name( char **buf, size_t *size, const x509_sequence *subject_alt_name );
